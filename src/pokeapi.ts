...
export class Pokeapi {
  ...
  async fetchPokemon(pokemonName: string): Promise<Pokemon> {
    ...
    if (cached) {
      console.log(`Cached pokemon: ${pokemonName}`);
      return cached.val;
    }
    try {
      ...
      throw new Error(`Failed to fetch pokemon: ${(error as Error).message}`);
    }
  }
}
...
export type Pokemon = {
  ...
  moves: {
    ...
    version_group_details: {
      ...
      move_learn_method: { ame: string; url: string };
      order: number;
    }[];
  }[];
  ...
  past_abilities: {
    generation: { name: string; url: string };
    abilities: { ability: any; is_hidden: boolean; slot: number }[];
  }[];
};
